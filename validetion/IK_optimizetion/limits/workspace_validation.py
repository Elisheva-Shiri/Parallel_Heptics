...
LIMITED_REST_ANGLE_KEYS = ("phi4", "phi5", "phi6")
PHI456_REST_LIMIT_DEG = 45.0
PHI456_REST_LIMIT_RAD = math.radians(PHI456_REST_LIMIT_DEG)
...
def nominal_rest_pose(model: dict) -> tuple[float, float, float]:
    """Rest P1 pose used as the zero-motion reference for angle limits."""
    z = max(0.0, min(6.0, float(model["lengths"]["d3"]) - 0.1))
    return (0.0, 0.0, z)


def rest_angles_for_phi1(phi1: float) -> tuple[dict[str, dict[str, float]] | None, tuple[float, float, float]]:
    """Return per-leg rest angles for this phi1, or None if rest is invalid."""
    rest_p1 = nominal_rest_pose(model)
    rest_solution = solve_all_legs(rest_p1, phi1, model=model)
    if not all(rest_solution[leg]["valid"] for leg in LEGS):
        return None, rest_p1
    return (
        {leg: {key: float(rest_solution[leg][key]) for key in ANGLE_KEYS} for leg in LEGS},
        rest_p1,
    )


def within_phi456_rest_limit(r: dict, rest_angles: dict[str, dict[str, float]], leg: str) -> bool:
    """True when phi4/phi5/phi6 are all within +/-45 deg from rest."""
    return all(
        abs(angle_diff(float(r[key]), float(rest_angles[leg][key]))) <= PHI456_REST_LIMIT_RAD
        for key in LIMITED_REST_ANGLE_KEYS
    )
...
print(f"Movement restriction: phi4/phi5/phi6 within +/-{PHI456_REST_LIMIT_DEG:g} deg from rest for all 3 legs")
...
for _ in range(N_SAMPLES):
    ...
    rest_angles, rest_p1 = rest_angles_for_phi1(phi1)
    if rest_angles is None:
        result = {leg: {"valid": False, "fail_reason": "rest_pose_invalid"} for leg in LEGS}
    else:
        result = solve_all_legs(P1, phi1, rest_angles=rest_angles, model=model)
    ik_latencies.append(time.perf_counter() - _t0)

    for leg in LEGS:
        r = result[leg]
        leg_is_valid = bool(r["valid"])
        if leg_is_valid:
            leg_is_valid = within_phi456_rest_limit(r, rest_angles, leg)
        if leg_is_valid:
            ...
            if not r["valid"]:
                ...
                reason = "phi456_outside_45deg_rest_window"
