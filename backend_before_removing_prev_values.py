...
class Experiment:
    ...
    def _calculate_motor_movements(self, finger_id: FingerId, direction: str, distance: int, motor_spacing: float = 1000.0) -> list[MotorMovement]:
        """
    ...
    def _technosoft_control_loop(self):
        ...
        MOVEMENT_THRESHOLD = 1.0  # Minimum movement to trigger signal
        is_comparison = True
        ...
        while self._running:
            if self._virtual_object.is_interacting:
                ...
                stiffness_value = self._virtual_object.stiffness_value
                # TODO - Use stiffness value in movement value
                movement_value = 100

                motors = []
                
                print(f"---------- x={self._virtual_object.x}, prev_x={self._virtual_object.prev_x}, {x_movement=} ----------")
                print(f"---------- y={self._virtual_object.y}, prev_y={self._virtual_object.prev_y}, {y_movement=} ----------")
                
                # TODO (if needed) - support diagonal movement
                # Movement Check
                if (
                    (self._state != ExperimentState.COMPARISON)
                    or (abs(x_movement) < MOVEMENT_THRESHOLD and abs(y_movement) < MOVEMENT_THRESHOLD)
                ):
                    if is_comparison:
                        is_comparison = False
                        motors.extend([
                    ...
                    # Must be in comparison, at least one crossed the movement threshold
                    is_comparison = True
                    ...
                    elif abs(y_movement) > MOVEMENT_THRESHOLD:
