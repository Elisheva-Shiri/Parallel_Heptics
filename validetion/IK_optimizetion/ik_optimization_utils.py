...
def evaluate_workspace(
    ...
    *,
    ...
    # Mild tie-break preference for a more compact platform offset (d1).
    score = metrics["volume_proxy_mm3"] + 0.1 * n_component - 0.01 * d1_mm
    if not accepted:
        score *= 0.1
...
def save_overview_plots(
    ...
    ax.set_title("Best score per d2/d3 pair (max over d1 platform offset)")
    ...
    # Mark the existing/real design (d1=4.0, d2=11.0, d3=9.5) for comparison.
    ax.scatter(
    ...
    ax.set_title("Workspace width/depth acceptance — existing design vs sweep")
    ax.legend(loc="best")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    ...
    compare = pd.concat(
        [
            pd.DataFrame([current_row]).assign(label="existing"),
    ...
    fig.suptitle("Existing design vs top suggestions")
    fig.tight_layout()
...
def render_and_checkpoint(task: tuple) -> str:
    ...
    finished writing all eight PNGs plus the summary JSON, so a config folder
