...
def make_figure(d: pd.DataFrame, out_stem: Path, *, bin_cm: float, dpi: int,
                width_in: float = 3.5, annotate: str | None = None) -> list[Path]:
    plt.rcParams.update({
    ...
    for ax in heat_axes[1:]:
        ax.set_yticklabels([])
    cb = fig.colorbar(im, ax=heat_axes, shrink=0.55, pad=0.02, aspect=14)
...
def main(argv=None) -> int:
    ...
    paths = make_figure(d, out_dir / "fig13_group_occupancy_paper", bin_cm=args.bin_cm, dpi=args.dpi,
    ...
    if args.paper_media is not None:
        ...
        dst = args.paper_media / "fig13_group_occupancy.png"
